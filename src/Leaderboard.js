...
const Leaderboard = ({updateP})=> {
    ...
    const listPlayers = numDescending?.map((player)=>{
        return (<PlayerCard key={player.id} username={player.username} picture={player.img_profile} totalMissed={player.total_missed} totalHits={player.total_hits} totalClicks={player.total_total} score={player.score} uA={player.accuracies} />)
    })
